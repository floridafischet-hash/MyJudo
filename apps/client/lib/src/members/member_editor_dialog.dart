        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              if (!_creating && widget.member != null) ...[
                const SizedBox(height: 20),
                _MemberProfileInfo(member: widget.member!),
              ],
...
class _MemberProfileInfo extends StatelessWidget {
  const _MemberProfileInfo({required this.member});
  ...
  Widget build(BuildContext context) {
    final rows = <MapEntry<String, String>>[
      if (member.highestGraduation != null &&
          member.highestGraduation!.isNotEmpty)
        MapEntry('Aktueller Grad', member.highestGraduation!),
      if (member.lastGraduationDate != null)
        MapEntry('Letzte Prüfung', _formatDate(member.lastGraduationDate!)),
      if (member.graduationsThisYear != null && member.graduationsThisYear! > 0)
        MapEntry('Prüfungen in diesem Jahr', '${member.graduationsThisYear}'),
      if (member.email != null && member.email!.isNotEmpty)
        MapEntry('E-Mail', member.email!),
      if (member.phone != null && member.phone!.isNotEmpty)
        MapEntry('Telefon', member.phone!),
      if (_address(member).isNotEmpty) MapEntry('Adresse', _address(member)),
      if (member.nationality != null && member.nationality!.isNotEmpty)
        MapEntry('Nationalität', member.nationality!),
    ];
    if (rows.isEmpty) return const SizedBox.shrink();
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Text('Mitgliedsdaten', style: Theme.of(context).textTheme.labelLarge),
        const SizedBox(height: 8),
        for (final row in rows)
          Padding(
            ...
            child: Row(
              children: [
                SizedBox(width: 170, child: Text(row.key)),
