...
class _HomeCalendarSummaryState extends State<HomeCalendarSummary> {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      ...
      if (widget.showUpcoming)
        _box(context, 'Kommende Termine', _upcomingChildren()),
    ],
  );

  List<Widget> _upcomingChildren() {
    final all = _allUpcoming();
    final shown = all.take(widget.maxUpcoming).toList();
    if (shown.isEmpty) return [const Text('Keine kommenden Termine.')];
    return [
      ...shown.map(
        (entry) => ListTile(
          contentPadding: const EdgeInsets.symmetric(vertical: 5),
      ...
      if (widget.onViewAllUpcoming != null && all.length > shown.length) ...[
        ...
        Align(
          ...
          child: TextButton.icon(
            ...
            label: const Text('Alle Termine im Kalender ansehen'),
          ),
        ),
      ],
    ];
  ...
  List<_UpcomingEntry> _allUpcoming() {
    return <_UpcomingEntry>[
      ...events.map(
