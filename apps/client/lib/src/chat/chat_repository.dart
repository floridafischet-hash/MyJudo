...
class ChatRepository {
  ...
  Future<ChatMessage> sendVoice(
    ...
    try {
      ...
      if (durationMs != null) fields['durationMs'] = durationMs.toString();
      if (replyToId != null) fields['replyToId'] = replyToId;
      final formData = FormData.fromMap(fields);
      final response = await _dio.post<Map<String, dynamic>>(
        '/chats/$chatId/voice',
  ...
  Future<void> uploadChatAvatar(
    ...
    try {
      ...
      await _dio.post<void>('/chats/admin/$id/avatar', data: data);
    } on DioException catch (error) {
      throw ChatApiException(_messageFor(error));
    }
  }

  Future<void> deleteChatAvatar(String id) async {
    try {
      await _dio.delete<void>('/chats/admin/$id/avatar');
    } on DioException catch (error) {
      throw ChatApiException(_messageFor(error));
    }
  }
